...
from ..manifest_validations import (
    ...
    _clean_extensions,
    _clean_required_pmtraders_version,
    _parse_version,
)
...
def test_clean_required_pmtraders_version(required_version, version, satisfied):
    cleaned = _clean_required_pmtraders_version(required_version, False, version)
    assert cleaned == {"constraint": required_version, "satisfied": satisfied}


def test_clean_required_pmtraders_version_optional():
    assert _clean_required_pmtraders_version(None, False) is None


@pytest.mark.parametrize(
    "required_version", ["3.8-3.9", "^3w.11", {"wrong", "data type"}, 3, 3.12]
)
def test_clean_required_pmtraders_version_with_invalid_range(required_version):
    with pytest.raises(ValidationError) as error:
        _clean_required_pmtraders_version(required_version, False, "3.12.1")
    assert error.value.code == AppErrorCode.INVALID.value


def test_clean_required_pmtraders_version_raise_for_pmtraders_version():
    with pytest.raises(ValidationError) as error:
        _clean_required_pmtraders_version("^3.13", True, "3.12.1")
    assert error.value.code == AppErrorCode.UNSUPPORTED_pmtraders_VERSION.value
