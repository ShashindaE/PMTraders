...
@allow_writer()
def handle_plugin_webhook(request: pmtradersContext, plugin_id: str) -> HttpResponse:
...
def handle_global_plugin_webhook(
    request: pmtradersContext, plugin_id: str
) -> HttpResponse:
    manager = get_plugins_manager(allow_replica=False)
...
def handle_plugin_per_channel_webhook(
    request: pmtradersContext, plugin_id: str, channel_slug: str
) -> HttpResponse:
    manager = get_plugins_manager(allow_replica=False)
