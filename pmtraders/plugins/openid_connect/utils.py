...
def get_user_from_oauth_access_token_in_jwt_format(
    ...
    is_staff_id = pmtraders_STAFF_PERMISSION
    if (use_scope_permissions and audience_in_token) or is_staff_email:
        ...
        if not permissions and token_permissions:
            permissions = get_pmtraders_permissions_from_list(token_permissions)
...
def get_pmtraders_permissions_qs_from_scope(scope: str) -> QuerySet[Permission]:
    scope_list = scope.lower().strip().split()
    return get_pmtraders_permissions_from_list(scope_list)


def get_pmtraders_permissions_from_list(permissions: list) -> QuerySet[Permission]:
    pmtraders_permissions_str = [s for s in permissions if s.startswith("pmtraders:")]
    if pmtraders_STAFF_PERMISSION in pmtraders_permissions_str:
        pmtraders_permissions_str.remove(pmtraders_STAFF_PERMISSION)
    if not pmtraders_permissions_str:
        return Permission.objects.none()

    permission_codenames = [
        perm.replace("pmtraders:", "") for perm in pmtraders_permissions_str
    ]
    permissions = get_permissions_from_codenames(permission_codenames)
    return permissions


def get_pmtraders_permission_names(permissions: QuerySet) -> list[str]:
