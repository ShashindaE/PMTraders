...
def setup_celery_logging(loglevel=None, **_kwargs):
    ...
    """
    logging.getLogger("pmtraders.schedulers").setLevel(loglevel)
...
class PersistentScheduler(celery.beat.PersistentScheduler, BaseScheduler):
    ...
    Usage:
    $ celery --app pmtraders.celeryconf:app beat \
        --scheduler pmtraders.schedulers.schedulers.PersistentScheduler
    """
...
class DatabaseScheduler(BaseDatabaseScheduler, BaseScheduler):
    ...
    Usage:
    $ celery --app pmtraders.celeryconf:app beat \
        --scheduler pmtraders.schedulers.schedulers.DatabaseScheduler
    """
