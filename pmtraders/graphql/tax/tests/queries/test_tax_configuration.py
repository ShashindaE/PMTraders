...
from pmtraders.tax.models import TaxConfiguration
...
def _test_field_resolvers(tax_configuration: TaxConfiguration, data: dict):
    ...
    assert (
        data["useWeightedTaxForShipping"]
        == tax_configuration.use_weighted_tax_for_shipping
    )
    ...
    assert (
        data["countries"][0]["useWeightedTaxForShipping"]
        == country_exception.use_weighted_tax_for_shipping
    )
