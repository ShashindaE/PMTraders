...
JWT_pmtraders_OWNER_NAME = "pmtraders"
...
def jwt_user_payload(
    ...
    token_owner: str = JWT_pmtraders_OWNER_NAME,
) -> dict[str, Any]:
...
def create_token(payload: dict[str, Any], exp_delta: datetime.timedelta) -> str:
    payload.update(jwt_base_payload(exp_delta, token_owner=JWT_pmtraders_OWNER_NAME))
...
def is_pmtraders_token(token: str) -> bool:
    """Confirm that token was generated by pmtraders not by plugin."""
    try:
        payload = jwt.decode(token, options={"verify_signature": False})
    except jwt.PyJWTError:
        return False
    owner = payload.get(JWT_OWNER_FIELD)
    if not owner or owner != JWT_pmtraders_OWNER_NAME:
        return False
    return True
...
def create_access_token_for_app(
    ...
    App can use user's JWT token to proceed given operation in pmtraders.
