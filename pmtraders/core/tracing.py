...
from ..core.telemetry import Link, Scope, SpanKind, pmtraders_attributes, tracer
...
def traced_atomic_transaction():
    with transaction.atomic():
        with tracer.start_as_current_span("transaction") as span:
            span.set_attribute(pmtraders_attributes.COMPONENT, "orm")
            yield
...
def otel_trace(span_name, component_name):
    with tracer.start_as_current_span(span_name) as span:
        span.set_attribute(pmtraders_attributes.COMPONENT, component_name)
        yield
...
def webhooks_otel_trace(
    ...
    with tracer.start_as_current_span(
        ...
        span.set_attribute(pmtraders_attributes.COMPONENT, "webhooks")
        span.set_attribute(
            pmtraders_attributes.pmtraders_WEBHOOK_EXECUTION_MODE, "sync" if sync else "async"
        )
        span.set_attribute(pmtraders_attributes.pmtraders_WEBHOOK_PAYLOAD_SIZE, payload_size)
        yield span
